...
describe("GET /health – price_source_circuits", () => {
  test("includes an entry per source that has a circuit breaker", async () => {
    ...
    expect(sourceNames).toEqual(
      expect.arrayContaining(["coingecko", "coinmarketcap"]),
    );
...
describe("GET /health – response shape", () => {
  ...
  test("database field reflects a successful database ping", async () => {
    jest.resetModules();
    const app = loadApp();
...
describe("GET /health – webhook_retry_worker queue depth", () => {
  ...
  test("reports pending retries, last batch size, and average delivery latency", async () => {
    ...
    expect(res.status).toBe(200);
    expect(res.body.jobs.webhook_retry_worker).toEqual(
...
describe("GET /health – rate limiting", () => {
  test("returns 429 after the configured per-IP health limit", async () => {
    ...
    try {
      ...
      expect(mockHealthCache.redis.incr.mock.calls[0][0]).toContain(
        "ratelimit:health:",
      );
    } finally {
