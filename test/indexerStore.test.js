        ...
        if (prop === "exec") {
          return async () => {
            ...
            for (const [cmd, args] of queue)
              results.push([null, await mockRedis[cmd](...args)]);
            return results;
          };
        }
        return (...args) => {
...
const mockRedis = {
  ...
  scard: jest.fn(async (key) => (mockSets.get(key) || new Set()).size),
...
describe("indexer event store", () => {
  ...
  test("saveEvent skips claim projection reads for non-claim events (#386)", async () => {
    await eventStore.saveEvent(
      baseEvent({
        id: "evt-non-claim",
        event_name: "recipient_added",
        data: { airdrop_id: "drop-1", recipient: "GRECIPIENT", amount: "10" },
      }),
    );

    expect(mockRedis.get).not.toHaveBeenCalledWith(
      "indexer:recipient:GRECIPIENT:claims",
    );
  });

  test("saveEvents skips claim projection reads for non-claim events (#386)", async () => {
    await eventStore.saveEvents([
      baseEvent({
        id: "evt-batch-non-claim",
        event_name: "recipient_added",
        data: { airdrop_id: "drop-1", recipient: "GRECIPIENT", amount: "10" },
      }),
    ]);
  ...
  test("upsertRecipient stores each recipient as its own hash field, not a whole-list read/write (#353)", async () => {
    ...
    expect(mockRedis.hset).toHaveBeenCalledWith(
      "indexer:airdrop:drop-hash:recipients",
      "GONE",
    ...
    expect(recipients.find((r) => r.recipient === "GONE")).toMatchObject({
      status: "pending",
      amount: "100",
    });
    expect(recipients.find((r) => r.recipient === "GTWO")).toMatchObject({
      status: "claimed",
      amount: "200",
    });
  });
