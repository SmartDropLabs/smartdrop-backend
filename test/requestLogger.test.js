...
jest.mock("../src/config", () => ({ slowRequestThresholdMs: 1000 }));
...
describe("requestLogger middleware", () => {
  ...
  test("includes the mount path without logging the query string", async () => {
    const app = express();
    const mountedRouter = express.Router();
    mountedRouter.use(requestLoggerMiddleware);
    mountedRouter.get("/ok", (req, res) => res.json({ ok: true }));
    app.use("/v1", mountedRouter);

    await request(app).get("/v1/ok?api_key=super-secret-value");

    expect(logger.info.mock.calls[0][1].path).toBe("/v1/ok");
    expect(JSON.stringify(logger.info.mock.calls[0][1])).not.toContain(
      "super-secret-value",
    );
  });
  ...
  describe("slow request warning (#244)", () => {
    test("does not log a slow-request warning for a fast request", async () => {
      ...
      const slowCall = logger.warn.mock.calls.find(
        ([message]) => message === "Slow request detected",
      );
    ...
    test("a slow request that also errors logs both the routine error line and the slow warning", async () => {
      ...
      hrtimeBigintSpy
        .mockReturnValueOnce(0n)
        .mockReturnValueOnce(2_000_000_000n);
