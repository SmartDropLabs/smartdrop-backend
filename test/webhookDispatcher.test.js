...
describe("dispatcher delivery success", () => {
  test("successful 200 marks delivery as success with attempts=1", async () => {
    ...
    expect(opts.headers["X-SmartDrop-Signature"]).toBe(
...
describe("dispatcher event-type filtering", () => {
  test("concurrent dispatches with the same event id deliver only once", async () => {
    await createWebhook();
    mockAxiosPost.mockResolvedValue({ status: 200 });

    const event = {
      event_type: "pool.assets_locked",
      event_id: "evt_concurrent_dedup",
      data: { pool_id: "p1" },
    };
  ...
  test("webhook filters narrow deliveries to matching pools", async () => {
    await createWebhook({
      url: "https://a.com",
      events: ["pool.assets_locked"],
      filters: { pool_id: "pool_1" },
    });
    await createWebhook({
      url: "https://b.com",
      events: ["pool.assets_locked"],
      filters: { pool_id: "pool_2" },
    });
    mockAxiosPost.mockResolvedValue({ status: 200 });
  ...
  test("captures error per-target when deliverToWebhook throws", async () => {
    const good = await createWebhook({
      url: "https://good.com",
      events: ["pool.assets_locked"],
    });
    const bad = await createWebhook({
      url: "https://bad.com",
      events: ["pool.assets_locked"],
    });
...
describe("dispatcher retry semantics", () => {
  test("reuses the webhook record across retries", async () => {
    const webhook = await createWebhook();
    const delivery = await deliveryRepo.create({
      webhook_id: webhook.id,
      event_id: "evt_cached_webhook",
      event_type: "pool.assets_locked",
    });
    ...
    const findById = jest.spyOn(webhookRepo, "findById");
  ...
  test("retry scheduling uses the backoff delay from the current attempt count", async () => {
    ...
    try {
      ...
      expect(delivery.next_retry_at).toBe(
        new Date(expectedRetryAt).toISOString(),
      );
      expect(zsets.get("webhooks:retries").get(delivery.id)).toBe(
        expectedRetryAt,
      );
    } finally {
...
describe("thundering-herd prevention across a real dispatch tick (#128)", () => {
  test("many deliveries failing at the same attempt within one tick get spread next_retry_at values", async () => {
    ...
    const nextRetryAtValues = new Set(
      results.map((r) => r.delivery.next_retry_at),
    );
...
describe("shouldRetry decision table", () => {
  test("retries on network error", () =>
    expect(dispatcher.shouldRetry(null, true)).toBe(true));
  test("retries on 500", () =>
    expect(dispatcher.shouldRetry(500, false)).toBe(true));
  test("retries on 503", () =>
    expect(dispatcher.shouldRetry(503, false)).toBe(true));
  test("retries on 408", () =>
    expect(dispatcher.shouldRetry(408, false)).toBe(true));
  test("retries on 429", () =>
    expect(dispatcher.shouldRetry(429, false)).toBe(true));
  test("does not retry on 400", () =>
    expect(dispatcher.shouldRetry(400, false)).toBe(false));
  test("does not retry on 404", () =>
    expect(dispatcher.shouldRetry(404, false)).toBe(false));
  test("does not retry on 200", () =>
    expect(dispatcher.shouldRetry(200, false)).toBe(false));
});
...
describe("request id propagation into webhook deliveries (issue #250)", () => {
  ...
  test("stamps the originating request id onto the delivery record", async () => {
    ...
    const [{ delivery }] = await requestContext.run(
    ...
    await expect(deliveryRepo.findById(delivery.id)).resolves.toEqual(
      expect.objectContaining({ request_id: "req_from_api" }),
    );
  });
